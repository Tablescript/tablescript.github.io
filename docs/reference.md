...
String work like most other languages. Single quotes or double quotes, whatever you like. Template strings (with back-ticks) use interpolation (see below).
...
Back-tick strings are treated as templates and anything inside `${` and `}` is evaluated as a standard expression.
